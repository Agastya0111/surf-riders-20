...
export const purchaseItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.kind === "surfboard") {
      const { data: board } = await supabase.from("surfboards").select("*").eq("key", data.itemKey).maybeSingle();
      if (!board) throw new Error("Surfboard not found");
      const { data: owned } = await supabase.from("owned_surfboards")
      ...
      await supabase.from("owned_surfboards").insert({ user_id: userId, surfboard_key: data.itemKey });
      return { ok: true };
    }

    const { data: item } = await supabase.from("shop_items").select("*").eq("key", data.itemKey).maybeSingle();
    if (!item) throw new Error("Item not found");
    const { data: owned } = await supabase.from("owned_items")
    ...
    await supabase.from("owned_items").insert({ user_id: userId, item_key: data.itemKey });
    return { ok: true };
  });
...
export const unlockSkill = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const { data: skill } = await supabase.from("skills").select("*").eq("key", data.skillKey).maybeSingle();
    if (!skill) throw new Error("Skill not found");
    const { data: have } = await supabase.from("user_skills")
...
export const claimDailyReward = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: profile } = await supabase.from("profiles").select("coins, gems").eq("id", userId).maybeSingle();
    await supabase.from("profiles").update({
